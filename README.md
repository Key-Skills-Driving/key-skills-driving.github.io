...
- **My style (Settings):** tone, length, a sign-off, free-text instructions, and up to three examples of the instructor's own writing. Examples come from breakdowns they fixed by hand (the app offers to keep one) or saved breakdowns ticked "Use as an example of my style". They're sent with each request so the AI matches the voice while the base rules keep it professional; "Tidy me up" vs "Close to how I write" sets how much it polishes. Styles stay on the phone (and in backups); the school server only passes them through, capped.
